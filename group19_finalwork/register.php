<?php
	...
	$lname = clean($_POST['myEmail']);
	$login = clean($_POST['myName']);
	$password = clean($_POST['myPword2']);
	...
	if($errflag) {
		...
		session_write_close();
		header("location: index.php?id=failed_register");
		exit();
	}

	//Create INSERT query
	$qry = "INSERT INTO members(lastname, login, passwd) VALUES('$lname','$login','".md5($_POST['myPword2'])."')";
	$result = @mysqli_query($GLOBALS["___mysqli_ston"], $qry);

	//Check whether the query was successful or not
	if($result) {
		header("location: index.php?id=success_register");
		exit();
	}else {
		die("Query failed");
	}
