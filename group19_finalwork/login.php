<?php
	...
	$password = clean($_REQUEST['myPword']);
	...
	if($errflag) {
		...
		session_write_close();
		header("location: index.php?id=form");
		exit();
	}

	//Create query
	$qry="SELECT * FROM members WHERE login='$login' AND passwd='".md5($_REQUEST['myPword'])."'";
	...
	if($result) {
		if(mysqli_num_rows($result) == 1) {
			...
			$_SESSION['ERRMSG_ARR'] = $errmsg_arr;
			header("location: index.php?id=failed");
			exit();
		}
	}else {
		die("Query failed");
